...
import slugshot from "./Projectile/slugshot";
...
export default class Barrel extends ObjectEntity {
    ...
    public shoot() {
        ...
        switch (this.definition.bullet.type) {
            ...
            case 'slugshot' :  {
                const scatterAngle = (Math.PI / 180) * this.definition.bullet.scatterRate * (Math.random() - .5) * 10;
                const bullet = new slugshot(this, this.tank, tankDefinition, this.definition.angle + scatterAngle + this.tank.positionData.values.angle);
            }
            default:
